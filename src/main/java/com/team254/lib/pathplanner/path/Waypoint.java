...
public record Waypoint(Translation2d prevControl, Translation2d anchor, Translation2d nextControl) {
    ...
    public static Waypoint autoControlFirstPoint(
        ...
        Translation2d velocitySqr =
                new Translation2d(
                        startVelocity.getNorm() * startVelocity.getNorm(),
                        startVelocity.getAngle());
    ...
    static Waypoint fromJson(JSONObject waypointJson) {
        Translation2d anchor =
                JSONUtil.translation2dFromJson((JSONObject) waypointJson.get("anchor"));
        Translation2d prevControl = null;
        Translation2d nextControl = null;

        if (waypointJson.get("prevControl") != null) {
            prevControl =
                    JSONUtil.translation2dFromJson((JSONObject) waypointJson.get("prevControl"));
        }
        if (waypointJson.get("nextControl") != null) {
            nextControl =
                    JSONUtil.translation2dFromJson((JSONObject) waypointJson.get("nextControl"));
        }
