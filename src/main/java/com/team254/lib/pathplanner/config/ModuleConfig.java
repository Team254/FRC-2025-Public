package com.team254.lib.pathplanner.config;
...
public class ModuleConfig {
    ...
    /**
     * The amount of motor torque lost while driving. Calculated by getting the torque of the motor
     * at the motor's max speed under load.
     */
     ...
     * @param maxDriveVelocityMPS The max speed that the drive motor can reach while actually
     *     driving the robot at full output, in M/S.
     * @param wheelCOF The coefficient of friction between the drive wheel and the carpet. If you
     *     are unsure, just use a placeholder value of 1.0.
     ...
     * @param numMotors The number of motors per module. For swerve, this is 1. For differential,
     *     this is usually 2.
     */
    public ModuleConfig(
        ...
        double maxSpeedCurrentDraw =
                this.driveMotor.getCurrent(this.maxDriveVelocityRadPerSec, 12.0);
        this.torqueLoss =
                Math.max(
                        this.driveMotor.getTorque(
                                Math.min(maxSpeedCurrentDraw, this.driveCurrentLimit)),
                        0.0);
    }
     ...
     * @param maxDriveVelocity The max speed that the drive motor can reach while actually driving
